...
import spchpy.libhmm as libhmm
import spchpy.utils as u
...
#    if prob_style:
#        self.set_probstyle(prob_style)
        
    f, ax = plt.subplots(1,2,figsize=figsize,sharey=True,constrained_layout=True)
    if prob_style == "prob":
        vmn = 1.e-5
        vmx = 1.0
    else:
...
def plot_trellis(self,X=None,plot_frameprobs=False,xticks=[],yticks=[],cmap=[],vmin=-10.,vmax=0.,fmt=".3f",figsize=(15,5),
    ...
    if not xticks:
        try:
            ...
            xticks = np.array(['t'+str(i) for i in range(0,n_frames)])
    if not yticks:
        yticks = self.states
    ...
    f,axt = plt.subplots(figsize=figsize)
