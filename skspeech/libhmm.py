...
class _BaseHMM():
    ...
    def __init__(self,n_states=1,transmat=[],startprob=[],states=[],prob_style="lin",prob_floor=PROB_FLOOR):
    ...
    def viterbi_trellis(self,X,col_norm=False):
        ...
        # backtracking    
        backtrace = np.ndarray(n_samples,'int')
        ...
        backtrace[n_samples-1] = best_end_state
        for i in range(n_samples-1,0,-1):
            backtrace[i-1]=backptrs[i,backtrace[i]]

        if col_norm:
            return frameprobs, trellis, backptrs, backtrace, scale_vec 
        else:
            return frameprobs, trellis, backptrs, backtrace
...
class DirectHMM(_BaseHMM):
    ...
    """       
    def __init__(self,n_states=1,n_features=2,transmat=[],startprob=[],states=[],prob_style="lin",prob_floor=PROB_FLOOR):
...
class DHMM(_BaseHMM):
    ...
    def __init__(self,n_states=1,transmat=[],startprob=[],states=[],prob_style="lin",prob_floor=PROB_FLOOR,n_features=1,emissionprob=[],n_symbols=[],labels=[]):
        _BaseHMM.__init__(self,n_states=n_states,transmat=transmat,startprob=startprob,states=states,prob_style=prob_style,prob_floor=prob_floor)
    ...
    def print_observation_model(self,labels=[]): 
        """ 
...
class Trellis():
        ...
        def reset(self):
            ...
            self.backtrace = np.zeros(self.n_samples,dtype='int') - 1 
        ...
        def viterbi_pass(self,X,hmm):
            ...
            # backtrace
            self.backtrace[self.n_samples-1] = self.end_state
            for i in range(self.n_samples-1,0,-1):
                self.backtrace[i-1]=self.backptrs[i,self.backtrace[i]]
            

        def print(self,Xlabels=[],Slabels=[],float_format=None,col_norm=False):
        ...
        #        display(HTML(dftrellis.to_html(float_format=float_format)))
            ...
            print("BACKPOINTERS and BACKTRACE")
            df1 = pd.DataFrame(self.backptrs.T,index=Slabels,columns=Xlabels)
            df2 = pd.DataFrame(self.backtrace.reshape(1,self.n_samples),index=["BT"],columns=Xlabels)
